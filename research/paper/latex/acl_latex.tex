...
We build a layered retrieval pipeline that combines exact verse lookup, chapter-aware database queries, numeric keyword suppression, and hybrid semantic ranking, and evaluate it with two complementary metrics: Recall@5 for retrieval coverage and citation\_support (an LLM-as-judge score, 0--1) for answer faithfulness.
...
Failure analysis shows that the dominant remaining errors trace to retrieval scope---semantic drift in thematic queries and within-chapter verse selection---not to citation misuse, which establishes that retrieval and faithfulness are separable failure modes in Bible RAG systems.
\end{abstract}
...
\begin{itemize}
  ...
  \item A \textbf{citation-faithfulness evaluation protocol} using an LLM-as-judge to score how faithfully retrieved verses appear in generated answers, producing a citation\_support score (0--1) and a false\_premise\_refusal flag (0/1).
  \item \textbf{Empirical results} on gold\_40 v0.3: R@5\,=\,0.941, mean citation\_support\,=\,0.978, unsupported claim rate\,=\,0.000, false-premise refusal rate\,=\,1.000.
...
Once retrieval scope is correct, the system cites retrieved verses without misattribution.
...
A recurring finding is that retrieval quality caps answer quality: systems cannot generate faithful answers from content they did not retrieve.

Our work extends this line: retrieval quality and answer faithfulness are separable, and a system can retrieve the right chapter while citing the wrong verse within it.
...
G-Eval \citep{liu2023geval} demonstrates strong correlation between GPT-4 judgments and human ratings for faithfulness-like criteria across summarisation and dialogue tasks.
We follow this line by using a Gemini judge to score citation\_support, and we add false\_premise\_refusal as an orthogonal robustness metric.

A known limitation of LLM-as-judge is same-family bias: a judge tends to rate outputs from the same model family higher \citep{zheng2023judging}.
Our judge (Gemini) evaluates answers generated by a Gemini-family model; we discuss this in the Limitations section.
...
\citet{hu2023falsepremise} construct a dataset of false-premise questions and show that current systems often answer as if the false premise were true rather than detecting and correcting it.
...
\textbf{Pass 1 (verse-level):} Regex matches \texttt{BOOK CHAPTER:VERSE} patterns (e.g., ``John 3:16'', ``Ps.\ 23:1''). An alias map normalises common abbreviations and alternate spellings to canonical book IDs.
...
\textbf{Layer 3 --- Semantic re-ranking within chapter (\texttt{selectWithinChapters}).} Fetched chapter verses are re-ranked by cosine similarity to the query embedding. An IVFFlat index on the \texttt{embedding} column provides approximate nearest-neighbour search. The top-$k$ results are selected.

\textbf{Per-chapter coverage guarantee.} After semantic re-ranking, the system verifies that at least one verse from each referenced chapter appears in the final candidate set. If not, it appends the chapter's first verse. This ensures multi-hop queries referencing two chapters always return at least one verse from each.
...
We pass the assembled verse context and original query to a Gemini model with a system prompt that instructs it to: answer from the provided verses only; cite specific verses inline; refuse or correct false premises; and decline to fabricate verses or attributions.

\subsection{Version History}

Table~\ref{tab:versions} shows how retrieval performance evolved across system versions.
v3 on v0.2 vs.\ v2 on v0.2 (same dataset, different architecture): R@5 holds at 0.882.
The architecture change fixes aion\_035 (Psalm 23 + John 10 multi-hop) but correctly re-breaks aion\_036, which was an accidental success in v2 due to an unrestricted semantic fallback.
...
\begin{table}[t]
  ...
  \begin{tabular}{llcc}
    ...
    \bottomrule
  \end{tabular}
  \caption{Retrieval performance across system versions. The v0.3 gain (+0.059 R@5) is attributable to annotation expansion for two questions (aion\_023, aion\_033), not architecture change.}
  \label{tab:versions}
\end{table}
...
\textbf{Dataset:} \texttt{aion\_bibleqa\_gold\_40\_v0.3.jsonl} --- 40 questions, 6 categories (Table~\ref{tab:categories}).
...
All 40 questions were verified against the live \texttt{bible\_verses} table (BSB translation).
...
\textbf{Judge model:} \texttt{gemini-3.1-flash-lite}.

For each row, the judge receives: (1) the question, (2) the category, (3) expected behaviour from the dataset annotation, (4) the retrieved verse block with full verse texts, and (5) the system answer.
...
\textbf{Implementation:} All 40 rows judged in a single pass with three-attempt retry per row and exponential backoff; 0 judge call failures.
...
Embedding model: pgvector extension with IVFFlat index on the \texttt{embedding} column.
Answer model: Gemini (same model family as judge; see the Limitations section), deployed as a Supabase Edge Function on Deno Deploy infrastructure.
Judge model: \texttt{gemini-3.1-flash-lite}.
Benchmark runner: calls the live Edge Function over HTTPS with SSE streaming from a consumer MacBook.
All benchmark runs are frozen as JSONL artefacts; results are reproducible from those frozen files.
...
Table~\ref{tab:main} shows overall metrics for v3 on gold\_40 v0.3.
R@5\,=\,0.941, citation\_support\,=\,0.978, and fp\_refusal\,=\,1.000 together show that where the system retrieves the right content, it uses that content without citation error.
...
Table~\ref{tab:percat} shows results by question category.
...
\begin{table}[t]
  ...
  \begin{tabular}{lccccc}
    ...
    multi\_hop    &  5 & 0.800 & 0.600 & 0.900 & --- \\
    false\_premise &  4 & ---   & ---   & ---   & 1.000 \\
    adversarial   &  2 & ---   & ---   & ---   & 1.000 \\
    \bottomrule
  \end{tabular}
  \caption{Per-category results. cs = citation\_support; fp = fp\_refusal.}
  \label{tab:percat}
\end{table}

\textbf{Direct and interpretive categories are solved.} R@5\,=\,1.00 and citation\_support\,=\,1.00 for all 17 questions. The exact verse lookup layer handles named verse queries completely; semantic re-ranking never introduces wrong verses when the reference is explicit.
...
\textbf{False-premise and adversarial robustness is perfect.} All six refusal-category questions scored 1. No fabricated content appeared in any answer.
...
Table~\ref{tab:percat} shows this: multi\_hop R@5\,=\,0.800 while multi\_hop citation\_support\,=\,0.900.
...
Both metrics are necessary. High R@5 does not prevent citation misuse; high citation\_support does not mean relevant content was retrieved. The Phase 3 evaluation protocol measures both.
...
\begin{table}[t]
  ...
  \begin{tabular}{p{2.3cm}p{2.3cm}p{2.3cm}}
    ...
    \bottomrule
  \end{tabular}
  \caption{Remaining failure taxonomy. None is a citation misuse failure.}
  \label{tab:failures}
\end{table}

In every case where retrieval returned relevant content, the answer model cited it correctly.
The engineering target is retrieval scope, not citation behaviour.

\subsection{The aion\_027 Grace Drift}

``Grace'' in Pauline letters has two semantic neighbourhoods: salvific grace (EPH.2.8--9: ``For by grace you have been saved through faith'') and epistolary grace (greeting formulas of the form ``Grace and peace to you\ldots'').
The epistolary cluster dominates the current semantic search surface: these short, formulaic verses have embeddings closer to the unqualified query ``grace'' than the longer salvific passage.

Two candidate fixes: (1) query expansion, prepending ``saved by'' or ``salvation'' to thematic grace queries at retrieval time; (2) salutation suppression, a post-retrieval filter penalising verses matching the pattern ``Grace and peace to you from X''.
The second is more targeted; the first generalises more broadly to other semantic drift cases.
...
The v4 fix is a \texttt{search\_verses\_in\_chapter(query\_embedding, book\_id, chapter, k)} Supabase RPC running constrained vector search within a single chapter.
For JHN.10, a query embedding for ``God as shepherd'' would rank JHN.10.11 above JHN.10.1.
The per-chapter guarantee becomes the top result of this per-chapter search rather than the chapter's first verse.
This is a well-defined engineering task.
...
Our v3 retrieval system, combining exact verse lookup, direct chapter-aware database queries, and hybrid semantic re-ranking, achieves R@5\,=\,0.941 on gold\_40 v0.3.
...
Once the right verses reach the context window, the system cites them correctly.
...
Future priorities: human annotation of a judge-scored sample; expansion to 200+ questions sampled from real users; thematic query expansion for grace-like semantic drift (v3.1); and a per-chapter vector search RPC for within-chapter verse selection (v4).
...
\textbf{LLM-as-judge, same model family.} Citation-faithfulness scores come from a Gemini judge evaluating Gemini-generated answers. Same-family judges tend to be lenient toward outputs from the same model family \citep{zheng2023judging}. The true citation\_support may be lower than 0.978 under a human annotator or cross-family judge. Future work should validate a sample of judgments against human ratings.

\textbf{Pilot benchmark size.} gold\_40 v0.3 contains 40 questions. Statistical uncertainty is high: one question is 2.5\% of R@5. Results in the multi\_hop category (n\,=\,5) and adversarial category (n\,=\,2) are especially noisy. A 95\% CI on multi\_hop R@5\,=\,0.800 with n\,=\,5 spans roughly $\pm$0.30.

\textbf{Constructed benchmark, not user-sampled.} Questions target known failure modes from v1 development, not the distribution of queries real users ask. Real-world R@5 may differ.

\textbf{Single translation (BSB).} All verses come from the Berean Standard Bible. Results may not generalise to systems using KJV, NIV, ESV, or other translations, where verse wording differs and embeddings shift.

\textbf{IVFFlat non-determinism.} Across three repeated runs on gold\_40 v0.2, three questions showed R@5 variance (aion\_006, aion\_007, aion\_036). Run-to-run R@5 ranged from 0.853 to 0.882 on the same question set. Benchmark runs in this paper were each performed once and frozen as canonical JSONL artefacts; results are reproducible from those frozen files.

\textbf{No ablation study.} The paper reports v3 as a whole system. The v1$\to$v2$\to$v3 progression provides coarse evidence that each change improved performance, but the relative contribution of each retrieval layer is not quantified.

%% ---------------------------------------------------------------
\section*{Acknowledgments}
%% ---------------------------------------------------------------

We thank the Berean Bible translators for making the BSB publicly available under a generous licence, which made this corpus-based study possible.
