...
Failure analysis shows that the remaining errors trace to retrieval scope (semantic drift in thematic queries and within-chapter verse selection), not to citation misuse, which establishes that retrieval and faithfulness are separable failure modes in Bible RAG systems.
\end{abstract}
...
\textbf{Direct and interpretive categories are solved.} R@5\,=\,1.00 and citation\_support\,=\,1.00 for all 17 questions. The exact verse lookup layer handles all named verse queries; semantic re-ranking never introduces wrong verses when the reference is explicit.
...
Whenever retrieval returned relevant content, the answer model cited it correctly.
...
Future priorities:
\begin{itemize}
  \item Human annotation of a judge-scored sample to validate citation\_support calibration.
...
We release Aion-BibleQA, the retrieval system, the judge harness, and all benchmark artefacts alongside this paper.
...
This study uses the Berean Standard Bible (BSB), a publicly available translation released under a licence permitting research use.
...
The benchmark targets a low-risk religious reference task with no plausible pathway to harm from retrieval failures.
