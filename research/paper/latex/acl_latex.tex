...
\begin{itemize}
  ...
  \item \textbf{Empirical results} on gold\_40 v0.3 using a Gemini LLM-as-judge citation protocol (Table~\ref{tab:rubric}): R@5\,=\,0.941, mean citation\_support\,=\,0.978, unsupported claim rate\,=\,0.000, false-premise refusal rate\,=\,1.000\,(6/6).
...
\textbf{Direct and interpretive categories.} R@5\,=\,1.00 and citation\_support\,=\,1.00 for all 17 questions (Table~\ref{tab:percat}). The exact verse lookup layer handles all named verse queries; semantic re-ranking does not introduce wrong verses when the reference is explicit.
...
Both metrics are necessary: high R@5 does not prevent citation misuse, and high citation\_support does not confirm that retrieved content was relevant.
...
The v4 fix is a \texttt{search\_verses\_in\_chapter} Supabase RPC running constrained vector search within a single chapter, parameterised by query embedding, book ID, chapter number, and return count $k$.
