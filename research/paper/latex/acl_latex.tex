...
We build a layered retrieval pipeline that combines exact verse lookup, chapter-aware database queries, numeric keyword suppression, and hybrid semantic ranking, and evaluate it with two metrics: Recall@5 for retrieval coverage and citation\_support (an LLM-as-judge score, 0--1) for answer faithfulness.
...
G-Eval \citep{liu2023geval} demonstrates strong correlation between GPT-4 judgments and human ratings for faithfulness criteria across summarisation and dialogue tasks.
...
Our judge (Gemini) evaluates answers generated by a Gemini-family model.
...
v3 on gold\_40 v0.3 achieves R@5\,=\,0.941, citation\_support\,=\,0.978, and fp\_refusal\,=\,1.000 (Table~\ref{tab:main}), establishing that retrieval correctness predicts citation correctness: the system cites retrieved verses correctly when retrieval succeeds.
...
\textbf{Direct and interpretive categories are solved.} R@5\,=\,1.00 and citation\_support\,=\,1.00 for all 17 questions (Table~\ref{tab:percat}). The exact verse lookup layer handles all named verse queries; semantic re-ranking never introduces wrong verses when the reference is explicit.
